...
% st = 100;
% t = 0:ss:FT;
% random noise for roll motion: 
 r = 0.025 + 0.0025*randn(FT/ss+1,1);
% plot(t,r);
...
for k = 0:ss:FT
    ...
    ui = u0;
% Euler's method:    
    xdot = Bluefin4DOFModel02(x,ui);
    x    = x + ss*xdot;
% add some random noise for roll:    
%    x(7) = x(7)+r(index);
...
% store data:
    data(index,1)  = ui(1); % rudder (commanded)
    data(index,2)  = ui(2); % propeller rpm (commanded)
    data(index,3)  = x(1);  % u [m/s]
    data(index,4)  = x(2);  % v [m/s]
    data(index,5)  = x(3);  % p [rad/s]
    data(index,6)  = x(4);  % r [rad/s]
    data(index,7)  = x(5);  % x-position
    data(index,8)  = x(6);  % y-position
    data(index,9)  = x(7);  % roll [rad]
    data(index,10) = x(8);  % yaw [rad]
    data(index,11) = k;     % time [s]
    data(index,12) = x(9);  % delta [rad]
    data(index,13) = x(10); % propeller [rps]
    data(index,14) = x(11); % bow thruster [rps]
end
...
% plotting
figure(1)
subplot(611);plot(data(:,11),data(:,3));grid
ylabel('u [m/s]');
subplot(612);plot(data(:,11),data(:,4));grid
ylabel('v [m/s]');
subplot(613);plot(data(:,11),data(:,5)*180/pi);grid
ylabel('p [deg/s]');
subplot(614);plot(data(:,11),data(:,6)*180/pi);grid
ylabel('r [deg/s]');
subplot(615);plot(data(:,11),data(:,10)*180/pi);grid
ylabel('\psi [deg]');
subplot(616);plot(data(:,11),data(:,9)*180/pi);grid
ylabel('\phi [deg]');
