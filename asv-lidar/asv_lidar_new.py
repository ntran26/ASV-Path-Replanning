...
class Lidar:
    ...
    def scan(self, pos, hdg, obstacles=None, map_border=None) -> np.ndarray:
        ...
        for idx, angle in enumerate(self._angles):
            ...
            sector_output = np.zeros(LIDAR_PARTITION)
            ...
            for i in range(LIDAR_PARTITION):
                ...
                sector_output[i] = pooled
            
            self._sector_feasible[i] = pooled > 0
            
        # return self._ranges.copy()
        return sector_output.copy()
    ...
    def render(self, surface: pygame.Surface):
        ...
        sector_size = BEAM_PER_SECTOR
        free_color = (90, 90, 200)      # blue
        blocked_color = (255, 60, 60)   # red
        ...
        for idx, angle in enumerate(self._angles):
            ...
            sector_idx = idx // sector_size
            
            if hasattr(self, "_sector_feasible"):
                if not self._sector_feasible[sector_idx]:
                    color = blocked_color     
                else:
                    color = free_color      
            else:
            ...
            # Highlight first beam of each sector
            is_boundary_beam = (idx % sector_size == 0)
            width = 3 if is_boundary_beam else 1

            # Draw beam
            pygame.draw.aaline(surface, color, (self._pos_x, self._pos_y), (x, y))
            if width > 1:
                pygame.draw.line(surface, color, (self._pos_x, self._pos_y), (x, y), width)
