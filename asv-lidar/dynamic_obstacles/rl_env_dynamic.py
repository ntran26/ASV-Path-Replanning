...
COLLISION_RANGE = 10
MOVING_OBS_SIZE = 40
MOVING_OBS_SPEED = 2.0
DYN_PREDICT_HORIZON = 8.0
DYN_TCPA_EPS = 1e-6
...
class ASVLidarEnv(gym.Env):
    ...
    def __init__(
        ...
        self.num_moving_obs = NUM_MOVING_OBS
    ...
    def generate_one_moving_obstacle(self):
        """Generate one moving square obstacle with constant-velocity motion."""
        for _ in range(100):
            x = np.random.randint(80, self.map_width - 80)
            y = np.random.randint(120, self.map_height - 220)
            if np.linalg.norm([x - self.start_x, y - self.start_y]) < 120:
                continue
            if np.linalg.norm([x - self.goal_x, y - self.goal_y]) < 120:
                continue

            heading_choices = np.array([0.0, 45.0, 90.0, 135.0, 180.0, 225.0, 270.0, 315.0])
            heading = float(np.random.choice(heading_choices))
            return {
                "x": float(x),
                "y": float(y),
                "size": float(MOVING_OBS_SIZE),
                "speed": float(MOVING_OBS_SPEED),
                "heading": heading
            }

        return {
            "x": float(self.map_width / 2),
            "y": float(self.map_height / 2),
            "size": float(MOVING_OBS_SIZE),
            "speed": float(MOVING_OBS_SPEED),
            "heading": 180.0
        }

    def _moving_obs_polygon(self, obs):
        half = obs["size"] / 2.0
        x = obs["x"]
        y = obs["y"]
        return [
    ...
    def update_moving_obstacles(self, dt):
        for obs in self.moving_obstacles:
            vx = obs["speed"] * np.sin(np.radians(obs["heading"]))
            vy = -obs["speed"] * np.cos(np.radians(obs["heading"]))
            ...
            if obs["x"] - half <= 5 or obs["x"] + half >= self.map_width - 5:
                obs["heading"] = (-obs["heading"]) % 360
                bounced = True
            if obs["y"] - half <= 5 or obs["y"] + half >= self.map_height - 5:
                obs["heading"] = (180 - obs["heading"]) % 360
                bounced = True

            if bounced:
                obs["x"] = np.clip(obs["x"], half + 6, self.map_width - half - 6)
                obs["y"] = np.clip(obs["y"], half + 6, self.map_height - half - 6)

    def _closest_moving_obstacle_metrics(self):
        if not self.moving_obstacles:
            return np.inf, np.inf, 0.0, "none"

        own_pos = np.array([self.asv_x, self.asv_y], dtype=np.float64)
        own_v = float(self.model._v)
        own_vel = np.array([
            own_v * np.sin(np.radians(self.asv_h)),
            -own_v * np.cos(np.radians(self.asv_h))
        ], dtype=np.float64)

        best = (np.inf, np.inf, 0.0, "none")

        for obs in self.moving_obstacles:
            obs_pos = np.array([obs["x"], obs["y"]], dtype=np.float64)
            obs_vel = np.array([
                obs["speed"] * np.sin(np.radians(obs["heading"])),
                -obs["speed"] * np.cos(np.radians(obs["heading"]))
            ], dtype=np.float64)

            rel_pos = obs_pos - own_pos
            rel_vel = obs_vel - own_vel
            rel_speed_sq = float(np.dot(rel_vel, rel_vel))

            if rel_speed_sq < DYN_TCPA_EPS:
                tcpa = np.inf
                dcpa = float(np.linalg.norm(rel_pos))
            else:
                ...
                if tcpa < 0:
                    ...
                    closest_rel = rel_pos + tcpa * rel_vel
                    dcpa = float(np.linalg.norm(closest_rel))

            bearing = (np.degrees(np.arctan2(rel_pos[0], -rel_pos[1])) - self.asv_h + 180) % 360 - 180
            heading_diff = (obs["heading"] - self.asv_h + 180) % 360 - 180

            if abs(bearing) <= 15 and abs(abs(heading_diff) - 180) <= 30:
                encounter = "head_on"
            elif -112.5 <= bearing <= -15:
                encounter = "crossing_starboard"
            elif 15 <= bearing <= 112.5:
                encounter = "crossing_port"
            elif abs(bearing) > 112.5 and abs(heading_diff) <= 30:
                encounter = "overtaking"
            else:
                encounter = "none"

            if tcpa < best[0]:
                best = (tcpa, dcpa, bearing, encounter)
    ...
    def reset(self,seed=None, options=None):
        ...
        # Generate one moving obstacle
        self.moving_obstacles = [self.generate_one_moving_obstacle() for _ in range(self.num_moving_obs)]
    ...
    def step(self, action):
        ...
        self.update_moving_obstacles(UPDATE_RATE)
        ...
        lidar_obstacles = list(self.obstacles) + [self._moving_obs_polygon(obs) for obs in self.moving_obstacles]
        self.lidar.scan((self.asv_x, self.asv_y), self.asv_h, obstacles=lidar_obstacles, map_border=self.map_border)
        ...
        # dynamic collision-risk reward based on TCPA/DCPA
        tcpa, dcpa, rel_bearing, encounter = self._closest_moving_obstacle_metrics()
        r_dyn = 0.0
        if np.isfinite(tcpa) and tcpa < DYN_PREDICT_HORIZON:
            r_dyn -= np.exp(-0.35 * max(tcpa, 0.0))
            r_dyn -= np.exp(-0.10 * max(dcpa, 0.0))

        # COLREG-inspired preference: in head-on or crossing-from-starboard, prefer starboard turn
        r_colregs = 0.0
        if encounter in ["head_on", "crossing_starboard"]:
            r_colregs += 0.25 * rudder
        elif encounter == "crossing_port":
            r_colregs -= 0.05 * abs(rudder)
        elif encounter == "overtaking":
            r_colregs += 0.10 * abs(rudder)
        ...
        if np.any(self.lidar.ranges.astype(np.int64) <= self.collision):
            ...
            reward = lambda_ * r_pf + (1 - lambda_) * r_oa + r_heading + r_exist + r_goal + 0.6 * r_dyn + r_colregs
    ...
    def render(self):
        ...
        for obs in self.moving_obstacles:
            pygame.draw.polygon(self.surface, (255, 140, 0), self._moving_obs_polygon(obs))
        ...
        if self.status is not None:
            tcpa, dcpa, _, encounter = self._closest_moving_obstacle_metrics()
            tcpa_txt = "inf" if not np.isfinite(tcpa) else f"{tcpa:04.1f}"
            status, rect = self.status.render(f"{self.elapsed_time:005.1f}s  HDG:{self.asv_h:+004.0f}({self.asv_w:+03.0f})  TGT:{self.tgt:+004.0f}  TGT_HDG:{self.angle_diff:.2f} TCPA:{tcpa_txt} DCPA:{dcpa:04.1f} {encounter}",(255,255,255),(0,0,0))
