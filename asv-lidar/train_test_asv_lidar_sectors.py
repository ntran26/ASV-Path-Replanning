...
from rl_env_lidar_sectors import ASVLidarEnv, RPM_MAX, RPM_MIN

"""
Train:
  python train_test_asv_lidar_sectors.py --mode train --algo ppo --timesteps 1000000

Test (render):
  python train_test_asv_lidar_sectors.py --mode test --algo ppo
...
class FixedBenchmarkCallback(BaseCallback):
    def __init__(
        ...
        out_json: str = "benchmark_history_lidar_sectors.json",
        out_csv: str = "benchmark_summary_lidar_sectors.csv",
        verbose: int = 1,
    ):
...
def build_model(algo: str, env, num_envs: int):
    ...
    if algo == "ppo":
        return PPO(
            ...
            env,
            verbose=1,
            tensorboard_log="./ppo_log_lidar_sectors/",
    ...
    if algo == "sac":
        return SAC(
            ...
            env,
            verbose=1,
            tensorboard_log="./sac_log_lidar_sectors/",
            learning_rate=learning_rate,
            batch_size=batch_size,
            gamma=gamma,
...
def main() -> None:
    ...
    model_path = args.model_path or f"{algo}_asv_model_lidar_sectors.zip"
    ...
    if args.mode == "train":
        ...
        checkpoint_cb = CheckpointCallback(
            ...
            save_path="models_lidar_sectors",
            name_prefix=f"{algo}_model_lidar_sectors",
        ...
        benchmark_cb = FixedBenchmarkCallback(
            ...
            out_json="benchmark_history_lidar_sectors.json",
            out_csv="benchmark_summary_lidar_sectors.csv",
            verbose=1,
        )

        model.learn(
            total_timesteps=int(args.timesteps),
            tb_log_name=f"asv_lidar_sectors_{algo}",
    ...
    if args.mode == "test":
        ...
        with open("asv_data_lidar_sectors.json", "w") as f:
    ...
    if args.mode == "eval":
        ...
        with open("benchmark_eval_lidar_sectors.json", "w") as f:
            json.dump(result, f, indent=2)
