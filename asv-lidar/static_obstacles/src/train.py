...
DETAIL_HEADER = [
    ...
    "min_lidar_all", "p10_front", "mean_r_pf", "mean_r_oa", "mean_r_local", "mean_r_center",
    "mean_r_border", "mean_r_exist", "mean_lambda",
    "min_sector_range", "p10_sector_range", "mean_sector_pen", "min_front_clearance", "max_block_alpha",
    "mean_abs_local_target_cte", "reward_per_step", "collision_steps", "has_reward_info",
]
...
SUMMARY_HEADER = [
    ...
    "min_min_lidar_all", "min_p10_front", "mean_r_pf", "mean_r_oa", "mean_r_local", "mean_r_center",
    "mean_r_border", "mean_r_exist", "mean_lambda",
    "min_sector_range", "p10_sector_range", "mean_sector_pen", "min_front_clearance", "max_block_alpha",
...
def episode_metrics(episode: Episode, env: ASVLidarEnv) -> Dict[str, Any]:
    ...
    return {
        ...
        "mean_r_pf": rollout.mean(track("r_pf")),
        "mean_r_oa": rollout.mean(track("r_oa")),
        "mean_r_local": rollout.mean(track("r_local")),
        "mean_r_center": rollout.mean(track("r_center")),
        "mean_r_border": rollout.mean(track("r_border")),
        "mean_r_exist": rollout.mean(track("r_exist")),
        "mean_lambda": rollout.mean(track("lam")),
        "min_sector_range": rollout.smallest(track("min_sector_range")),
        "p10_sector_range": rollout.smallest(track("p10_sector_range")),
        "mean_sector_pen": rollout.mean(track("mean_sector_pen")),
        "min_front_clearance": rollout.smallest(track("front_clearance")),
        "max_block_alpha": rollout.largest(track("block_alpha")),
        "mean_abs_local_target_cte": rollout.abs_mean(track("local_target_cte")),
        "has_reward_info": int(bool(track("r_pf") or track("r_oa") or track("r_exist") or track("lam"))),
...
class EvalMetricsCallback(BaseCallback):
    ...
    def _summarise(self, ep_metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "mean_r_pf": avg("mean_r_pf"),
            "mean_r_oa": avg("mean_r_oa"),
            "mean_r_local": avg("mean_r_local"),
            "mean_r_center": avg("mean_r_center"),
            "mean_r_border": avg("mean_r_border"),
            "mean_r_exist": avg("mean_r_exist"),
            "mean_lambda": avg("mean_lambda"),
            "min_sector_range": worst("min_sector_range"),
            "p10_sector_range": worst("p10_sector_range"),
            "mean_sector_pen": avg("mean_sector_pen"),
            "min_front_clearance": worst("min_front_clearance"),
            "max_block_alpha": best("max_block_alpha"),
            "selection_score": float(selection_score),
            "reward_info_rate": avg("has_reward_info"),
        }
...
def side_path_guard(action: np.ndarray, obs: dict) -> np.ndarray:
    """Stop the policy steering into the worse side.

    Positive CTE means the vessel is left of the path and should recover to
    starboard; positive rudder turns to starboard.  When the recovery direction
    and the clearer side agree, forbid rudder in the opposite direction.
    """
    if not USE_SIDE_PATH_GUARD:
        return action

    front = float(obs["front_clearance"][0])
    if front <= GUARD_FRONT_MIN:
        return action

    cte = float(obs["cross_track_error"][0])
    local_target = float(obs["local_target_cte"][0])
    side_diff = float(obs["side_clearance_diff"][0])   # right minus left

    starboard_clearer = side_diff > SIDE_DIFF_GUARD_MIN
    port_clearer = side_diff < -SIDE_DIFF_GUARD_MIN

    if (cte > CTE_GUARD_MIN and starboard_clearer) or (local_target < -0.10 and starboard_clearer):
        action[0] = max(float(action[0]), MIN_CORRECTIVE_ACTION)
    elif (cte < -CTE_GUARD_MIN and port_clearer) or (local_target > 0.10 and port_clearer):
