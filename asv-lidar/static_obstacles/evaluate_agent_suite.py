"""
Evaluate one trained SB3 agent on the fixed ASV evaluation suite.

Edit the USER SETTINGS block at the top. Supported algorithms: sac, ppo, td3, ddpg.
"""
...
# User settings
# -----------------------------
# Choose one: "sac", "ppo", "td3", "ddpg"
ALGO = "ppo"
MODEL_PATH = "models/sac_model_1M.zip"
MODEL_PATH = "ppo_asv_model.zip"
SUITE_JSON = "eval_suite/asv_eval_suite.json"
OUT_DIR = f"eval_results/eval_suite_{ALGO}"
DETAIL_CSV = os.path.join(OUT_DIR, f"{ALGO}_eval_suite_details.csv")
DETAIL_JSON = os.path.join(OUT_DIR, f"{ALGO}_eval_suite_details.json")
SUMMARY_JSON = os.path.join(OUT_DIR, f"{ALGO}_eval_suite_summary.json")
SUMMARY_CSV = os.path.join(OUT_DIR, f"{ALGO}_eval_suite_summary.csv")
...
def load_model_by_algo(algo: str, model_path: str):
    algo = str(algo).lower()
    if algo == "sac":
        return SAC.load(model_path, device="auto")
    if algo == "ppo":
        return PPO.load(model_path, device="auto")
    if algo == "td3":
        return TD3.load(model_path, device="auto")
    if algo == "ddpg":
        return DDPG.load(model_path, device="auto")
    raise ValueError(f"Unknown ALGO={algo!r}. Use sac, ppo, td3, or ddpg.")
...
def evaluate_one(model, env: ASVLidarEnv, scenario: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        # Internal moment terms used to compute group-level pooled std_cte exactly.
        # They are removed before writing eval_suite_details.json/csv.
...
def main() -> None:
    ...
    model = load_model_by_algo(ALGO, MODEL_PATH)
    env = ASVLidarEnv(
        render_mode=None,
        map_width=MAP_WIDTH,
