"""
rl_env_reward_v2b.py
====================

``rl_env_reward_v2.py`` PLUS one additional fix: the obstacle-blocked gate
(``block_alpha``) is driven by a wide forward arc instead of the narrow +/-15
deg front window.

Motivation (from the asv_lidar.mp4 failure analysis)
----------------------------------------------------
In the recorded collision, the vessel turned until obstacle 1 left the +/-15
deg front window. ``front_clearance`` then jumped straight to LIDAR_RANGE
(16 m, "all clear") even though the obstacle was only 0.3 m off the starboard
beam. Because ``block_alpha`` is computed from ``front_clearance`` alone, it
collapsed to 0, which snapped ``gamma_e`` back from 0.05 (tolerant, bypassing)
to 0.20 (strict tracking). The restored path pull then steered the vessel back
into the obstacle it had just started to clear -- visible in the render as the
rudder reversing from -99.9 to +88.

Fix: compute a dedicated ``block_clearance`` as the MINIMUM sector range over
+/-BLOCK_ARC_DEG (60 deg default) and drive ``block_alpha`` from that. While
anything is close anywhere in the forward arc -- bow or beam -- the tolerant
gamma_e stays engaged and the path term does not yank the vessel back.

Scope / experiment hygiene
--------------------------
This is a SINGLE-VARIABLE change relative to rl_env_reward_v2.py. Specifically:

  * ``front_clearance`` is UNCHANGED (still the 10th-percentile over +/-15 deg),
    so that observation channel keeps exactly the semantics the 1M checkpoint
    was trained on. BLOCK_FRONT_DEG stays 15.0.
  * Only ``block_alpha`` switches to the wide-arc minimum. Note this does
    propagate into one observation channel, ``local_target_cte``
    (= side_cte_sign * BYPASS_CTE * block_alpha), which will now stay engaged
    while an obstacle is abeam. That is the intended behaviour.
  * Set BLOCK_USE_WIDE_ARC = False to fall back to exactly v2 behaviour.

If this variant still under-turns, the next thing to try is widening
BLOCK_FRONT_DEG from 15.0 to ~35.0 so the front_clearance OBSERVATION also
stops reading 16.0 with an obstacle on the beam. That is deliberately NOT done
here because it changes an observation the pretrained policy relies on, and
would confound this ablation.

Everything below this point is inherited from rl_env_reward_v2.py:
...
# --- REWARD_V2B: wide-arc blocked gate -------------------------------------
# block_alpha is driven by the MINIMUM sector range over +/-BLOCK_ARC_DEG
# rather than the 10th-percentile over the narrow +/-BLOCK_FRONT_DEG window.
# A narrow front window lets an obstacle sitting on the beam read as "clear",
# which collapses block_alpha and snaps gamma_e back to strict path tracking at
# exactly the wrong moment (see module docstring).
# Minimum, not percentile: a single close return in the arc must hold the gate.
BLOCK_USE_WIDE_ARC = True
BLOCK_ARC_DEG = 60.0
SIDE_CLEAR_TIE = 0.15
BYPASS_CTE = 0.5
K_LOCAL_TARGET = 1.0
...
class ASVLidarEnv(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        self.block_clearance = LIDAR_RANGE
    ...
    def _update_local_planner_features(self) -> None:
        ...
        # REWARD_V2B: gate on the wide forward arc so an obstacle on the beam
        # keeps the tolerant gamma_e engaged. front_clearance (observation) is
        # intentionally left untouched.
        if BLOCK_USE_WIDE_ARC:
            arc_mask = np.abs(sector_angles) <= BLOCK_ARC_DEG
            arc_vals = obs_d[arc_mask]
            self.block_clearance = float(np.min(arc_vals)) if arc_vals.size else float(LIDAR_RANGE)
        else:
            self.block_clearance = float(self.front_clearance)

        self.block_alpha = float(np.clip(
            (BLOCK_D_SAFE - self.block_clearance) / (BLOCK_D_SAFE - BLOCK_D_CRIT),
            0.0,
            1.0,
        ))
    ...
    def step(self, action):
        ...
        info = {
            ...
            "block_clearance": float(getattr(self, "block_clearance", LIDAR_RANGE)),
