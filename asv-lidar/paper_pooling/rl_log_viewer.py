...
# -----------------------------
# LiDAR / vessel constants
# -----------------------------
LIDAR_FULL_BEAMS = 720
LIDAR_FULL_STEP = 360 / LIDAR_FULL_BEAMS    # 0.5 degrees

try:
    from asv_lidar import LIDAR_SWATH, LIDAR_BEAMS, LIDAR_RANGE, LIDAR_SECTORS
except Exception:
    LIDAR_SWATH = 240
    LIDAR_BEAMS = 225
    LIDAR_RANGE = 16.0
...
try:
    from ship_model import VESSEL_LENGTH, VESSEL_WIDTH, LIDAR_OFFSET_M
except Exception:
    VESSEL_LENGTH = 1.7
    VESSEL_WIDTH = 0.5
...
def main() -> None:
    ...
    ap.add_argument("--test-case", type=int, default=None, help="Optional test_case id to draw reference path/obstacles in policy map coordinates")
    ...
    # Optional policy-style scenario overlay. When enabled, the first log pose is
    # mapped to the scenario start point, so the viewer uses the same coordinate
    # frame as the RL environment.
    scenario_start = None
    scenario_goal = None
    scenario_obstacles = []
    scenario_ref_path = None
    if args.test_case is not None:
        if TestCase is None:
            raise SystemExit("--test-case requires test_run.py to be available")
        scenario = TestCase()
        sx, sy, gx, gy = scenario.position(test_case=args.test_case)
        scenario_start = (float(sx), float(sy))
        scenario_goal = (float(gx), float(gy))
        scenario_obstacles = scenario.obstacles(test_case=args.test_case)
        n_path = max(40, int(np.hypot(float(gx) - float(sx), float(gy) - float(sy)) * 5.0))
        scenario_ref_path = np.column_stack((
            np.linspace(float(sx), float(gx), n_path, dtype=np.float32),
            np.linspace(float(sy), float(gy), n_path, dtype=np.float32),
        ))
    ...
    path_world: List[Tuple[float, float]] = []  # mapped into policy/test-case coordinates

    view_center_world = start_xy  # where the map camera is centered
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                # Map toggles
                    ...
                    if frame is not None:
                        ...
                        start_pt = scenario_start if scenario_start is not None else (0.0, 0.0)
                        path_world = [start_pt]
                        view_center_world = start_pt
        ...
        while not paused and now >= next_due:
            ...
            if next_frame is None:
                ...
                # Update map path. If --test-case is provided, map real-log displacement
                # into the RL scenario frame by anchoring the first pose at the
                # scenario start point. Otherwise use old relative-to-origin coords.
                ...
                if scenario_start is not None:
                    current_map = (scenario_start[0] + rel[0], scenario_start[1] + rel[1])
                else:
                    current_map = rel
                path_world.append(current_map)

                traj_xy.append((float(frame.x_m), float(frame.y_m)))
                traj_yaw.append(float(frame.yaw_deg))

                if follow_mode:
                    view_center_world = current_map
        ...
        if frame is None:
            ...
            if origin_world is None:
                rel_x, rel_y = start_xy
            else:
                rel_x = float(start_xy[0] + (frame.x_m - origin_world[0]))
                rel_y = float(start_xy[1] + (frame.y_m - origin_world[1]))
        ...
        if show_map:
            if frame is None or origin_world is None:
                policy_render.draw_policy_scene(
                    screen,
                    map_rect,
                ...
                screen.blit(small.render("waiting for frame...", True, (235, 235, 245)), (map_rect.left + 8, map_rect.top + 8))
            else:
                rel = (
                    float(frame.x_m - origin_world[0]),
                    float(frame.y_m - origin_world[1]),
                )
                current_map = (scenario_start[0] + rel[0], scenario_start[1] + rel[1]) if scenario_start is not None else rel
                raw_lidar = pick_lidar_swath(frame.lidar_m, lidar_draw_angles, index0_deg=LIDAR_INDEX_DEG)
                sector_ranges, sector_closeness, sector_angles = policy_render.pool_raw_lidar_to_sectors(
                    raw_lidar, n_sectors=int(LIDAR_SECTORS), max_range=float(LIDAR_MAX)
                )
                status_lines = [
                    f"points={len(path_world)}" + (f" test_case={args.test_case}" if args.test_case is not None else ""),
                    f"u={frame.u_body_mps:+.2f} v={frame.v_body_mps:+.2f} yaw_rate={frame.yaw_rate:+.1f}",
                    f"S1={frame.s1:.1f} S2={frame.s2:.1f}",
                ]
                policy_render.draw_policy_scene(
                    screen,
                    map_rect,
                    map_width=10.0 if scenario_start is not None else None,
                    map_height=25.0 if scenario_start is not None else None,
                    reference_path=scenario_ref_path,
                    obstacles=scenario_obstacles,
                    start_xy=scenario_start,
                    goal_xy=scenario_goal,
                    trajectory=path_world,
                    current_xy=current_map,
                    heading_deg=float(frame.yaw_deg),
                    raw_lidar_ranges=raw_lidar,
                    raw_lidar_angles=lidar_draw_angles,
                    ...
                    show_raw_lidar=True,
                    show_sector_lidar=True,
                    show_icon=True,
                )
                yy = map_rect.top + 8
                for line in status_lines:
                    screen.blit(small.render(line, True, (235, 235, 245)), (map_rect.left + 8, yy))
                    yy += 18
