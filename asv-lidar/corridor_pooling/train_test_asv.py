...
from rl_env import (
    ASVLidarEnv, DEFAULT_EVAL_LAMBDA,
    CRUISE_RPM, FIXED_RPM, RPM_DELTA, RPM_FLOOR, RPM_CEIL,
)
...
# 6 obstacle-count groups × 10 episodes = 60 eval episodes per checkpoint
...
def action_to_rpm(throttle_cmd: float) -> float:
    ...
    return float(np.clip(CRUISE_RPM + RPM_DELTA * throttle_cmd, RPM_FLOOR, RPM_CEIL))
...
class EvalMetricsCallback(BaseCallback):
    def __init__(
        ...
        *,
        ...
        self.header = [
            ...
            "min_front_clearance", "max_block_alpha", "mean_abs_local_target_cte",
            "weighted_success", "selection_score", "reward_info_rate",
        ]
        self.summary_header = [
            "timesteps", "mean_ep_reward", "std_ep_reward", "mean_ep_len",
            "success_rate", "collision_rate", "border_rate", "obstacle_rate", "timeout_rate",
        ...
        for obs_count in EVAL_OBS_COUNTS:
            self.summary_header.extend([
                f"obs_{obs_count}_success_rate",
                f"obs_{obs_count}_obstacle_rate",
                f"obs_{obs_count}_border_rate",
                f"obs_{obs_count}_timeout_rate",
                f"obs_{obs_count}_mean_abs_cte",
            ])
    ...
    def _on_step(self) -> bool:
        ...
        obs_0_s = group_summaries.get("obs_0_success_rate", 0.0)
        obs_1_s = group_summaries.get("obs_1_success_rate", 0.0)
        obs_2_s = group_summaries.get("obs_2_success_rate", 0.0)
        obs_3_s = group_summaries.get("obs_3_success_rate", 0.0)
        obs_4_s = group_summaries.get("obs_4_success_rate", 0.0)
        ...
        summary = {
            ...
            "mean_abs_local_target_cte": mean_of("mean_abs_local_target_cte"),
            "weighted_success": float(weighted_success),
