...
# Sector pooling mode used by the policy observation.
# Hardcoded for this branch so two branches can run simultaneously without
# relying on environment variables.
LIDAR_POOLING_MODE = normalise_pooling_mode("corridor")

# Safety-adjusted width used by both paper and corridor pooling.
# Default is the same width as the current inflated collision hull.
...
class Lidar:
    ...
    def pooling_mode(self):
        return LIDAR_POOLING_MODE

    def _update_sectors(self):
        self._sector_ranges, self._sector_closeness, self._sector_angles = pool_lidar_to_sectors_shared(
            self._ranges.astype(np.float32),
            raw_angles_deg=self._angles.astype(np.float32),
            lidar_range=LIDAR_RANGE,
            lidar_swath_deg=LIDAR_SWATH,
            n_sectors=LIDAR_SECTORS,
            safe_width_m=FEASIBILITY_SAFE_WIDTH,
            mode=LIDAR_POOLING_MODE,
        )
