...
from asv_lidar import (
    Lidar, LIDAR_RANGE, LIDAR_SECTORS, LIDAR_SWATH,
    LIDAR_POOLING_MODE, FEASIBILITY_SAFE_WIDTH,
)
...
# No log10_lambda observation is included in this comparison package.
LAMBDA_MIN = 1e-4
LAMBDA_MAX = 1.0
...
# Targeted corridor-pooling repair distribution.
# Goal: preserve obs_0 while improving obs_1/obs_2 obstacle collisions.
TRAIN_OBS_COUNTS = [0, 1, 2, 3, 4]
TRAIN_OBS_PROBS = [0.10, 0.25, 0.35, 0.20, 0.10]
...
class ASVLidarEnv(gym.Env):
    ...
    def _sample_lambda(self) -> None:
        # Fixed path-vs-obstacle reward balance for this comparison.
        # lambda_override is kept in the API for old scripts, but ignored.
    ...
    def _update_local_planner_features(self) -> None:
        # Front blockage must be obstacle-only.
        # This prevents pool walls/borders from creating false "obstacle ahead" cues.
        obs_d = self.lidar_reward.sector_ranges.astype(np.float32)

        # Side choice should still know about walls/borders.
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if self.test_case is None:
            if self.forced_num_obs is not None:
                ...
                num_obs = int(np.random.choice(TRAIN_OBS_COUNTS, p=TRAIN_OBS_PROBS))
        else:
            num_obs = 0
