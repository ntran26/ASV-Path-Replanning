...
# Path generation
PATH_MODE = "straight"    # quick mode: exact straight-path task first
CURVE_PROB = 0.0
...
# Quick single-obstacle mode
OBSTACLE_MODE = "single_on_path"   # switch to "old_path_relative" to restore old generator
OBSTACLE_SIZE = 1.0
OBSTACLE_PATH_START_FRAC = 0.25
OBSTACLE_PATH_END_FRAC = 0.75

USE_AUTO_DETOUR_PATH = True
DETOUR_SIDE = 1.0          # +1 = starboard/right relative to path tangent
DETOUR_OFFSET = 2.0        # [m]
DETOUR_APPROACH = 4.0      # [m]
DETOUR_TRIGGER_DIST = 0.9  # [m]
...
class ASVLidarEnv(gym.Env):
    ...
    def _sample_lambda(self) -> None:
        # Quick mode: fixed lambda unless explicitly overridden.
        # Random lambda conditioning is useful later, but it makes the single
        # obstacle debugging task harder than necessary.
        if self.lambda_override is not None:
            lam = float(self.lambda_override)
        else:
            lam = float(DEFAULT_EVAL_LAMBDA)
    ...
    def _recompute_path_metadata(self) -> None:
        if len(self.path) < 2:
            self.path_s = np.zeros(len(self.path), dtype=np.float32)
    ...
    def _generate_path(self, start_x: float, start_y: float, goal_x: float, goal_y: float) -> np.ndarray:
        ...
        self._recompute_path_metadata()
        return self.path

    def _obstacle_center(self, obs: List[Tuple[float, float]]) -> np.ndarray:
        return np.mean(np.array(obs, dtype=np.float32), axis=0)

    def _apply_single_obstacle_detour(self) -> None:
        """Smoothly offset the active reference path around a blocking obstacle."""
        if not USE_AUTO_DETOUR_PATH or len(self.obstacles) == 0 or len(self.path) < 3:
            return

        best_idx = None
        best_dist = float("inf")
        for obs in self.obstacles:
            c = self._obstacle_center(obs)
            d = np.linalg.norm(self.path - c[None, :], axis=1)
            idx = int(np.argmin(d))
            dist = float(d[idx])
            if dist < best_dist:
                best_idx = idx
        ...
        if best_idx is None or best_dist > DETOUR_TRIGGER_DIST:
            return

        s_obs = float(self.path_s[best_idx])
        s0 = max(0.0, s_obs - DETOUR_APPROACH)
        s1 = min(float(self.path_s[-1]), s_obs + DETOUR_APPROACH)
        if s1 <= s0 + 1e-6:
            return

        new_path = self.path.copy()
        for i, pnt in enumerate(self.path):
            s_val = float(self.path_s[i])
            if s_val < s0 or s_val > s1:
                continue
            tau = (s_val - s0) / (s1 - s0)
            lateral = DETOUR_SIDE * DETOUR_OFFSET * math.sin(math.pi * tau)
            tangent = self._path_tangent(i)
            # right/starboard normal in x-right, y-up coordinates
            right_normal = np.array([tangent[1], -tangent[0]], dtype=np.float32)
            q = pnt + lateral * right_normal
            q[0] = float(np.clip(q[0], 1.0, self.map_width - 1.0))
            q[1] = float(np.clip(q[1], 1.0, self.map_height - 1.0))
            new_path[i] = q

        self.path = new_path.astype(np.float32)
        self.path_mode_used = f"{self.path_mode_used}+detour"
        self._recompute_path_metadata()
    ...
    def _generate_single_on_path_obstacle(self) -> List[List[Tuple[float, float]]]:
        ...
        for idx, (px, py) in enumerate(self.path):
            s_val = float(self.path_s[idx]) if len(self.path_s) > idx else 0.0
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if self.test_case is None:
            num_obs = 1 if self.obstacle_mode == "single_on_path" else int(np.random.randint(0, self.max_obs + 1))
        else:
            num_obs = 0
        self.obstacles = self._generate_obstacles(num_obs, self.test_case)
        self._apply_single_obstacle_detour()
    ...
    def step(self, action):
        ...
        info = {
            ...
            "auto_detour": bool(USE_AUTO_DETOUR_PATH),
            "fixed_rpm": bool(FIXED_RPM),
