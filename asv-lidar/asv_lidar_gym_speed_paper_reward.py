...
# Approx. max forward speed at RPM_MAX (steady-state thrust = drag)
...
class ASVLidarEnv(gym.Env):
    ...
    def _get_obs(self):
        return {
            ...
            'spd_norm': np.array([min(max(self.model._v / max(U_MAX, 1e-6), 0.0), 1.5)], dtype=np.float32),
    ...
    def step(self, action):
        ...
        # Living penalty (encourages shorter trajectories)
        r_exist = -1.0

        # Heading/course alignment term: cos(chi_tilde) in [-1, 1]
        angle_diff_rad = np.radians(self.angle_diff)
        cos_chi = float(np.cos(angle_diff_rad))

        # Speed terms
        U = float(self.model._v)
        U_max = float(max(U_MAX, 1e-6))
        U_norm = float(np.clip(U / U_max, 0.0, 1.5))  # allow slight overspeed if dynamics permit

        # (Eq. 31-like) Path-following reward with velocity:
        # r_pf = -1 + (exp(-gamma_e*|y_e|) + 1) * (U_norm*cos(chi_tilde) + 1)
        # Here we approximate y_e using self.tgt (cross-track offset in pixels)
        gamma_e = 0.05
        y_e = float(abs(self.tgt))
        r_pf = -1 + (np.exp(-gamma_e * y_e) + 1.0) * (U_norm * cos_chi + 1.0)

        # (Eq. 32-like) Obstacle-avoidance reward (beam-weighted, front-prioritized)
        # We keep all beams (no sector pooling), but match the paper's "weighted average" idea.
        eps_x = 1.0          # distance offset to avoid singularity
        gamma_theta = 0.03   # beam-angle weighting strength
        lidar_d = self.lidar.ranges.astype(np.float32)
        lidar_theta = self.lidar.angles.astype(np.float32)

        weights = 1.0 / (1.0 + np.abs(gamma_theta * lidar_theta))
        inv_term = 1.0 / np.maximum(lidar_d - eps_x, 1e-3)
        r_oa = -float(np.sum(weights * inv_term) / (np.sum(weights) + 1e-6))

        # Goal / terminal shaping (paper-style: sparse terminal rewards)
        self.distance_to_goal = float(np.linalg.norm([self.asv_x - self.goal_x, self.asv_y - self.goal_y]))
        goal_radius = self.collision + 30
        r_goal = 0.0
        reached_goal = self.distance_to_goal <= goal_radius
        if reached_goal:
            r_goal = 100.0
        ...
        # Collision overrides (terminal failure reward)
        collided = bool(np.any(self.lidar.ranges.astype(np.float32) <= self.collision))
        if collided:
            reward = -100.0
